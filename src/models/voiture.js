...
exports.changeMYCar = async (ndp, id) => {
  return await db.execute(`UPDATE voiture SET ndp = ? WHERE voiture.id = ? ;`, [
    ndp, id]);
};

exports.getUserInfo = async (ndp) => {
  return await db.execute(
    `SELECT user.id , voiture.user_id , name , lastname , telephone FROM user INNER JOIN voiture
  ON user.id = voiture.user_id  where ndp = ?;`,
    [ndp]
  );
};
